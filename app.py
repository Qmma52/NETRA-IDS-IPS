...
from services.auth import require_login, logout_button, is_admin
...
def eval_rule(rule, row):
    ctx = {
        ...
        "file_integrity_changed": row.get("file_integrity_changed", False),
        "True": True,
        "False": False,
    }
    try:
        return bool(eval(rule["condition"], {"__builtins__": {}}, ctx))
    except Exception:
        return False
...
if "live_running" not in st.session_state:
    st.session_state.live_running = True  # default ON
...
# ✅ Start/Stop buttons (only affects timer streaming)
st.sidebar.divider()
...
edited_rules = st.sidebar.data_editor(rules_df, num_rows="dynamic", use_container_width=True, hide_index=True)

# thresholds
st.sidebar.divider()
st.sidebar.subheader("Detection thresholds (Demo)")
    ...
    header_box.markdown(
        ...
        <div class="header-row">
          <div class="header-left">
            <h1 style="margin-bottom: 0.25rem;">🛡️ NETRA IDS/IPS</h1>
...
if "csv_pos" not in st.session_state: st.session_state.csv_pos = 0
if "csv_sig" not in st.session_state: st.session_state.csv_sig = None
...
if mode == "Upload Logs (CSV)":
    ...
    if st.session_state.csv_sig != sig:
        ...
        # ✅ FIX: show first chunk immediately (so ips/events appear)
    ...
    # ✅ Safety: if someone reset to 0, still show first chunk
    ...
    # ✅ Only auto-advance when BOTH: stream_csv + live_running + refresh>0
    ...
    if total == 0:
        ...
        # ✅ FIX: loop logic should not wrap to 0 when csv_pos == total
        # and if modulo gives 0, jump to first chunk (not empty window).
...
with cB:
    ...
    if alerts_df.empty:
        ...
        st.bar_chart(ip_counts.set_index("src_ip"))
...
with b3:
    if st.button("📜 Show NETRA IDS/IPS Firewall Rules", use_container_width=True):
        res = list_ids_rules()
        if res.ok:
            st.code(res.message)
        else:
...
with st.expander("📘 How this NETRA IDS/IPS works "):
    st.markdown(
        """
**Flow**
1) Data Source → Demo events or CSV live stream  
2) Detection → Rules apply on each row  
...
**Important**
- Streamlit reruns on widget interaction (normal).
- Start/Stop controls only the timer-based live feed, so clicks won't keep forcing timer reruns.
"""
    )
...
if refresh and refresh > 0 and st.session_state.live_running:
    try:
        ...
        # fallback (not ideal, but only active while RUNNING)
        time.sleep(refresh)
        st.rerun()
